...
def fetch_feed(
    ...
    for i in range(retries):
        try:
            print(f"获取 Feed ({i+1}/{retries}): {final_url[:80]}...")
...
def _parse_item(item: Dict[str, Any], goal_username: str) -> Optional[Tweet]:
    ...
    tweet_id = _extract_tweet_id(url) or _extract_tweet_id(str(item.get("id", "")))
    if not tweet_id:
        return None
    ...
    if not quote_author:
        ...
        for link in links:
            if link.get("type") == "quote":
                ...
                if link_html:
                    author_match = re.search(r'<strong>([^<]+)</strong>', link_html)
                    if author_match:
                        quote_author = author_match.group(1).strip()
                break
